...
TOP_UNIVERSE = [
    ...
    "PEP", "T", "AXP", "ADBE", "DIS", "QCOM", "GS", "MRK", "NOW", "CAT"
]
EXCLUDED = {"INTU", "CRM", "AVGO"}
ACTIVE_UNIVERSE = [t for t in TOP_UNIVERSE if t not in EXCLUDED]
BENCHMARK = "SPY"
...
def get_clients():
    if not ALPACA_API_KEY or not ALPACA_SECRET_KEY:
        raise RuntimeError("Faltan ALPACA_API_KEY / ALPACA_SECRET_KEY en variables de entorno.")
...
def load_state() -> dict:
    if not os.path.exists(STATE_FILE):
        return {"stops": {}, "last_entry_week": None}
...
def add_indicators(weekly: pd.DataFrame, spy_weekly: pd.DataFrame) -> pd.DataFrame:
    w = weekly.copy()
    w["sma30"] = w["Adj Close"].rolling(SMA_WEEKS).mean()
    w["ema10"] = w["Adj Close"].ewm(span=EMA_EXIT_WEEKS, adjust=False).mean()
    w["vol_avg"] = w["Volume"].rolling(VOL_AVG_WEEKS).mean()
    w["box_high_prev"] = w["High"].shift(1).rolling(BOX_WEEKS).max()
    w["box_low_prev"] = w["Low"].shift(1).rolling(BOX_WEEKS).min()
    w["sma30_up"] = w["sma30"] > w["sma30"].shift(1)
...
def get_positions_map(trading: TradingClient) -> Dict[str, dict]:
...
def get_open_order_symbols(trading: TradingClient) -> set:
...
# SIGNALS
# =========================
def build_signals(data_map: Dict[str, pd.DataFrame]) -> tuple[Dict[str, pd.DataFrame], pd.Timestamp]:
    if BENCHMARK not in data_map:
        raise RuntimeError("No se pudo descargar SPY para fuerza relativa.")
...
def get_candidates(weekly_map: Dict[str, pd.DataFrame], positions_map: Dict[str, dict], open_order_symbols: set) -> List[SignalRow]:
...
# ORDERS
# =========================
def submit_buy(trading: TradingClient, symbol: str, qty: int) -> None:
...
def submit_sell(trading: TradingClient, symbol: str, qty: float) -> None:
    order = MarketOrderRequest(
        symbol=symbol,
        qty=qty,
        side=OrderSide.SELL,
...
def manage_exits(trading: TradingClient, weekly_map: Dict[str, pd.DataFrame], positions_map: Dict[str, dict], state: dict) -> None:
