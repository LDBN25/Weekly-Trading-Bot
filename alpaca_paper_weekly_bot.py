...
def build_signals(data_map: Dict[str, pd.DataFrame]):
...
def manage_exits(
    ...
    for symbol, pos in positions_map.items():
        ...
        latest_box_stop = (
            float(row["box_low_prev"])
            if not math.isnan(float(row["box_low_prev"]))
            else current_stop
        )
        ...
        if current_stop > 0 and daily_close <= current_stop:
            logging.info(
                f"[EXIT STOP] {symbol} qty={qty} stop={current_stop:.2f} close={daily_close:.2f}"
            )
            submit_sell(trading, symbol, qty)
            continue
...
def open_new_positions(
    ...
    for c in candidates:
        ...
        logging.info(
            f"[BUY] {c.symbol} shares={shares} ref_close={c.entry_ref_close:.2f} "
            f"stop={c.stop_price:.2f} score={c.score:.2f}"
        )
...
def run_once():
    ...
    account = trading.get_account()
    logging.info(
        f"[ACCOUNT] equity={account.equity} cash={account.cash} buying_power={account.buying_power}"
    )
